export function getCuilCuit(document_number?: any, gender?: any) {
   /**
   ...
   if (document_number.length != 8 || isNaN(document_number)) {
      if (document_number.length == 7 && !isNaN(document_number)) {
         ...
         /*  if (document_number.length == 6 && !isNaN(document_number)) {
        document_number = "00".concat(document_number);
      } */
         console.log('documentoooo erroor', document_number); // Muestro un error en caso de no serlo.
   ...
   //console.log("RESTOOO", resto);
   // console.log("indexxxx", HOMBRE.indexOf(gender));
   /*
